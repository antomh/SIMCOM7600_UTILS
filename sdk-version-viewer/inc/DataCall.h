...
typedef struct {
  ...
  char if_name[32];
  char ip_str[16];
  char ip_str_v6[INET6_ADDRSTRLEN];
  char pri_dns_str[16];
  char sec_dns_str[16];
  char pri_dns_str_v6[INET6_ADDRSTRLEN];
  char sec_dns_str_v6[INET6_ADDRSTRLEN];
  char gw_str[16];
  char gw_str_v6[INET6_ADDRSTRLEN];
  unsigned int mask;
...
extern int set_host_route_v6(char *old_ip, char *new_ip, char *if_name);
extern int set_route_dns(char *gw, char *pri_dns_addr, char *sec_dns_addr, char *if_name);
...
int start_dataCall_ext_v01(app_tech_e tech, int ip_family, int profile, char *apn, char *username, char *password, int auth);
